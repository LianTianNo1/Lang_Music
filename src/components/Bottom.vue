<template>
  <div class="bottom">
    <div class="music-box">
      ...
      <div
        class="img-wrap el-icon-arrow-up"
        @click="toSongDetail()"
        title="打开音乐详情页"
      ...
      <div class="music-info" v-show="globalMusicInfo.songName">
        <span class="music-name" :title="globalMusicInfo.songName">{{
          globalMusicInfo.songName
        }}</span>
    ...
    <div
      @click="showQueue($event)"
      ref="queue"
      title="播放队列"
      :class="{
        'delete-queue-style bofangliebiao': this.queueStyle == 'delete',
        'add-queue-style bofangliebiao': this.queueStyle == 'add',
        bofangliebiao: this.queueStyle == 'normal',
      }"
    >
        ...
        if (this.musicQueue.length == 1) {
          ...
          let prevIndex =
            (nowIndex - 1) % this.musicQueue.length < 0
              ? ((nowIndex - 1) % this.musicQueue.length) + this.musicQueue.length
